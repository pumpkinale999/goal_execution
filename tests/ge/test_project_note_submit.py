...
def _setup_with_note(client, *, project_note_id: str | None = TEST_PROJECT_NOTE_ID):
    ...
    created = create_project(
        ...
        U_PM,
        body,
        bootstrap_startup=project_note_id is not None,
    )
...
def test_material_submit_invalid_content_ref_format_returns_400(client):
    ...
    resp = client.post(
        ...
        json={
            "payload": {
                "summary": "report",
                "content_ref": f"kb:{TEST_PROJECT_NOTE_ID}#section",
            }
        },
    )
