...
def test_create_year_explicit_owner_not_actor(client):
    ...
    body = resp.json()
    assert body["owner_user_id"] == "u-other"
...
def test_create_year_omitted_or_blank_owner_falls_back_to_actor(client):
    ...
    assert omitted.json()["owner_user_id"] == "reviewer-1"
    ...
    assert blank.json()["owner_user_id"] == "reviewer-1"
    ...
    assert spaces.json()["owner_user_id"] == "reviewer-1"
