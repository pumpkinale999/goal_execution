...
def default_system_planned_due(
    now: str,
    phase_planned_end: str | None,
    *,
    phase_planned_start: str | None = None,
) -> str:
    """Prefer phase-window midpoint; else phase end; else now+30d."""
    if phase_planned_start and phase_planned_end:
        return midpoint_plan_date(phase_planned_start, phase_planned_end)
...
def is_protected_system_end_sign_prerequisite(
    ...
    *,
    ...
    if task is None or item is None:
        return False
    return (
        is_system_end_sign_task(task)
        and item.is_system
        and item.name in SYSTEM_END_GATE_ITEM_NAMES
    )
...
def _find_system_end_gate_item(db: Session, phase_id: str) -> GeGateItem | None:
    return (
        ...
        .filter(
            ...
            GeGateItem.name.in_(tuple(SYSTEM_END_GATE_ITEM_NAMES)),
        )
        .first()
    )
...
def _ensure_start_side(
    ...
    *,
    ...
    planned_due = default_system_planned_due(
        now,
        phase_planned_end,
    ...
    if start_gi is None:
        ...
        if start_gi.planned_due is None:
            start_gi.planned_due = planned_due
            start_gi.updated_at = now
        if legacy_complete and start_gi.status != "signed":
...
def _ensure_end_side(
    ...
    *,
    ...
    if end_gi is None:
        ...
        db.add(end_gi)
        gate_item_count += 1
    elif end_gi.planned_due is None:
        end_gi.planned_due = planned_due
        end_gi.updated_at = now
...
def seed_system_lifecycle_graph(
    ...
    *,
    ...
    end_gate_id: str,
    now: str,
    start_phase_planned_start: str | None = None,
    start_phase_planned_end: str | None = None,
    end_phase_planned_start: str | None = None,
    ...
    if seed_start:
        start_phase = db.get(GePhase, start_phase_id)
        start_ps = start_phase_planned_start or (start_phase.planned_start if start_phase else None)
        start_pe = start_phase_planned_end or (start_phase.planned_end if start_phase else None)
        added_tasks, added_gis = _ensure_start_side(
            ...
            project_note_id=project_note_id,
            phase_planned_start=start_ps,
            phase_planned_end=start_pe,
        )
        task_count += added_tasks
        gate_item_count += added_gis

    if seed_end:
        end_phase = db.get(GePhase, end_phase_id)
        end_ps = end_phase_planned_start or (end_phase.planned_start if end_phase else None)
        end_pe = end_phase_planned_end or (end_phase.planned_end if end_phase else None)
        added_tasks, added_gis = _ensure_end_side(
            ...
            now=now,
            project_note_id=project_note_id,
            phase_planned_start=end_ps,
            phase_planned_end=end_pe,
        )
        task_count += added_tasks
        gate_item_count += added_gis
